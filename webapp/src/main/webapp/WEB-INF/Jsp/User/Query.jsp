...
<table border="1" cellpadding="10" cellspacing="0">
    ...
    <c:forEach items="${requestScope.page.list}" var="list">
        <tr>
            ...
            <td><c:if test="${list.path != null}"><img width="50" height="50" src="<%=request.getContextPath()%>/path?head=<%=java.net.URLEncoder.encode((String) request.getAttribute("path"))%>"/></c:if></td>
        ...
        <c:if test="${page.pageNo -1!= 0}">
            <a href="<%=request.getContextPath()%>/query?pageNo=${page.pageNo -1}&pageSize=${page.pageSize}"><fmt:message key="index.shang"/></a>&nbsp;

        </c:if>
        <c:if test="${page.pageNo +1<= page.pageCount}">
            <a href="<%=request.getContextPath()%>/query?pageNo=${page.pageNo +1}&pageSize=${page.pageSize}"><fmt:message key="index.xia"/></a>&nbsp;
        </c:if>
        <a href="<%=request.getContextPath()%>/query?pageNo=${page.pageCount}&pageSize=${page.pageSize}"><fmt:message key="index.wei"/></a>&nbsp;
        &nbsp;
        <select id="pageSize">
