...
<form:form action="${pageContext.request.contextPath}/add" modelAttribute="user" method="post" enctype="multipart/form-data">
    <c:if test="${user.path != null}">
        <c:set value="${user.path}" var="path"   scope="request"/>
<img width="75" height="75" src="<%=request.getContextPath()%>/ResponseEntityHead?path=<%=java.net.URLEncoder.encode((String) request.getAttribute("path"))%>"/><br/></c:if>
    <fmt:message key="index.head"/>:<input type="file" name="head"/><br/>

    <!-- 如果有ID 就表示修改，修改是put请求  而且名字是不能修改的-->
    <c:if test="${user.id == 0}"><fmt:message key="index.name"/>:<form:input path="name"/><font color="red"><form:errors path="name"></form:errors></font><br/></c:if>
    ...
    <fmt:message key="index.sex"/>:<form:radiobuttons path="sex" items="${sex}"/><font color="red"><form:errors path="sex"></form:errors></font><br/>
